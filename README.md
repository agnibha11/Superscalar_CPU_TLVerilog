...
**ISA:** RV32I + Zicsr counters &nbsp;•&nbsp; **Issue width:** 2 (in-order) &nbsp;•&nbsp; **Branch predictor:** two-bit saturating &nbsp;•&nbsp; **Language:** TL-Verilog &nbsp;•&nbsp; **Platform:** [Makerchip IDE](https://makerchip.com)
...
4. [Baseline Core Operation](#4-baseline-core-operation)
   - [4.6 The two-bit saturating branch predictor](#46-the-two-bit-saturating-branch-predictor)
5. [Dual-Issue Architecture](#5-dual-issue-architecture)
6. [Implementation](#6-implementation)
   - [6.11 Porting the two-bit branch predictor](#611-porting-the-two-bit-saturating-branch-predictor)
7. [Hazards Introduced by Dual Issue](#7-hazards-introduced-by-dual-issue)
   - [7.7 Predictor behaviour under dual issue](#77-predictor-behaviour-under-dual-issue)
...
| **Branch prediction** | Two-bit saturating counter | Adds an early, stage-2 *predicted-taken* redirect alongside the stage-3 mispredict redirect, and adds a piece of shared predictor state that must be hoisted alongside the register file. |
...
### 4.6 The two-bit saturating branch predictor

Branches resolve only at the execute stage, three cycles after fetch, so the fetch unit must guess. The baseline contains the classic **two-bit saturating counter** (bimodal) predictor.

**The state machine.** A single two-bit register, `$BranchState`, encodes confidence rather than a bare last-outcome bit:

| State | Meaning | Prediction | Next state (taken / not-taken) |
|---|---|---|---|
| `2'b00` | Strongly not-taken | Not taken | `2'b01` / `2'b00` (saturates) |
| `2'b01` | Weakly not-taken | Not taken | `2'b10` / `2'b00` |
| `2'b10` | Weakly taken | Taken | `2'b11` / `2'b01` |
| `2'b11` | Strongly taken | Taken | `2'b11` (saturates) / `2'b10` |

The prediction is the most significant bit. The second bit provides *hysteresis*: a loop branch sitting in `2'b11` is moved only to `2'b10` by a single not-taken outcome, where it still predicts taken. A one-bit predictor would mispredict twice per loop — once on the exit and once on the next entry — while the two-bit counter absorbs the exit as a single mispredict. The counter resets to `2'b01`.

**The whole predictor** is these few lines:
...
- **It is one global counter, not a table.** There is no indexing by PC; every branch shares one counter.
- **Prediction is read with a `>>2` alignment.** The counter is written at stage 3 and read at stage 2; the two-transaction alignment guarantees a settled value and prevents a combinational loop.
- **The update is gated on commit.** `$branch_or_reset` asserts only when a branch retires on the good path, so wrong-path branches never pollute the predictor. `$RETAIN` is the TL-Verilog keyword for "hold the current value", implementing saturation.

**Two redirects, not one.** With a predictor present a branch can steer the PC at two different points, and the baseline defines a separate redirect condition for each:

1. **Predicted-taken, at stage 2** — as soon as decode identifies a branch and the counter predicts taken, fetch speculatively follows the target. Its mask term is `4'b1110`, clearing one bit, since it kills only the instruction already fetched behind the branch.
...
2. **Mispredict, at stage 3** — when the branch resolves, the outcome is compared against the prediction, and recovery now depends on the *direction* of the error:
...
A branch predicted not-taken but resolved taken redirects to `$branch_target`; one predicted taken but resolved not-taken redirects to `$pc_inc`. That second case does not exist in a fallthrough-only predictor.

> Both facts have direct consequences for the two-wide core: the state must be hoisted so one counter serves both slots, and the new stage-2 redirect must be split into slot-0 and slot-1 forms. See [Section 6.11](#611-porting-the-two-bit-saturating-branch-predictor).

### 4.7 Speculation control: the good-path mask
...
| Load-address and store-data formation | Branch-predictor state (`$BranchState`) |
| Branch prediction read and `$pred_taken_branch` | Forwarding-availability signals |
...
|fetch                        // one shared pipeline
   ...
   @3  slot-0 redirect summary (final), test pass/fail,
       two-bit branch-predictor state (ONE counter, two update steps)
   /instr[1:0]                // TWO slots built from ONE description
      ...
      @2  decode, operand read + 6-way forwarding, hazard detect, replay,
          branch prediction read + predicted-taken redirect
...
{$next_pc[31:0], $next_no_fetch} =
   ...
   $pred_taken_branch_cond_0 ? {/instr[0]>>1$branch_target, 1'b0} :
   $pred_taken_branch_cond_1 ? {/instr[1]>>1$branch_target, 1'b0} :
   $second_issue_cond        ? {>>0$Pc, 1'b0} :
   $no_fetch_cond            ? {>>0$Pc, 1'b1} :
              {$Pc + 32'd8, 1'b0};   // Default: sequential fetch of the next group of two.
```

The ordering embodies the priority rule: **a later pipeline stage wins**, and **within a stage, slot 0 precedes slot 1**. The default case is the two-instruction sequential stride. The two replay cases target the individual slot's own PC — the property on which the hazard resolution rests. The two predicted-taken cases sit immediately below the replays, preserving the baseline's relative priority (a replay is aborting and must win over a speculative prediction).
...
$next_good_path_mask[3+1:0] =
   {$GoodPathMask[3:0]
     ...
     // A predicted-taken branch is NON-aborting, so for either slot it clears
     // only the younger group -- the same term as slot 0.
...
### 6.11 Porting the two-bit saturating branch predictor

The predictor described in [Section 4.6](#46-the-two-bit-saturating-branch-predictor) required four coordinated changes. The guiding rule is the one that governs the register file and data memory: *the predictor is shared machine state, so exactly one copy must exist, and it must observe every committed branch in program order.*

**(1) Hoisting the counter, and two updates per cycle.** Left inside `/instr[1:0]`, the counter would have been replicated into two independent predictors, each seeing half the branch history — architecturally harmless but functionally useless. It is hoisted into `|fetch`.

Hoisting raises a question the single-issue core never faced: both slots may retire a branch in the same cycle, so the counter may need *two* updates at once. Three options were weighed:

| Option | Assessment |
|---|---|
| Add a structural hazard and replay slot 1 when both slots hold a branch | Consistent with the memory and CSR ports, but branches are frequent, so the IPC cost would be real and recurring. |
| Let one slot win and drop the other update | Cheapest, but the counter drifts from the true branch history and predicts less accurately. |
| Apply both updates in series within the cycle | Reproduces the baseline's semantics exactly, costs no issue slots, and is nearly free in area since the counter is two bits wide. **Chosen.** |

The decisive observation is that the predictor is **not architectural state**. A mis-updated counter costs performance, never correctness, so the structural hazard used for the memory and CSR ports would buy nothing here.
...
// committed branch, in program order" semantics.
...
Slot 0's step is applied first and slot 1's operates on its result, so the counter ends the cycle in exactly the state a single-issue core would reach after executing the same two branches consecutively. Both terms remain gated on `$commit`, so wrong-path branches never disturb the counter. `$RETAIN` is replaced by an explicit saturation constant because `$bp_state_0` is combinational rather than state.

> **Equivalence.** For all four counter states and all sixteen combinations of slot activity and branch outcome, the chained update produces the same result as applying the same retirements one at a time in program order. The two-step formulation is an exact reproduction of the baseline predictor, not an approximation.

**(2) Per-slot prediction from the shared counter.** The read side stays inside the replicated scope, since each slot decides for its own instruction, but reads the one shared counter:
...
The `>>2` alignment is preserved exactly; only the scope prefix changes. Since a fetch group advances the pipeline by one cycle, this now reaches two *groups* back rather than two instructions back — an inherent property of a wider machine, and the reason both slots of a group share one prediction.

**(3) Splitting the predicted-taken redirect.** The new stage-2 redirect is treated like every other redirect condition — split per slot, qualified by the good-path mask, and for slot 1 additionally qualified by the slot-0 summary:
...
Its mask term is `4'b1110` for **both** slots — the only symmetric redirect pair in the design. The reason is instructive: a predicted-taken branch is **non-aborting**, so the branch itself commits normally and neither slot's own group may be killed. Contrast the replay pair, where slot 0 uses `4'b1100` because a replaying instruction must kill itself, while slot 1 uses `4'b1110` so slot 0 survives.

Killing slot 1 of the *same* group when slot 0 predicts taken is handled not by the mask — which cannot distinguish the slots — but by adding `$pred_taken_branch` to the stage-2 slot-0 summary, so `$killed_by_slot0` removes the fall-through instruction. This single term is easy to overlook and essential: without it, a predicted-taken branch in slot 0 would redirect the fetch stream while its own slot-1 neighbour continued to execute and commit.

**(4) Direction-aware mispredict recovery.** The mispredict target must now account for predictions in both directions:
...
Because `$pc_inc` is already per-slot, each slot recovers to its own fall-through address, so a mispredicted branch in slot 1 resumes at its own PC+4 rather than slot 0's.

**Summary of the port**

| Baseline element | Dual-issue treatment |
|---|---|
| `$BranchState` counter | Hoisted to `\|fetch`; one counter, two chained saturating steps (slot 0 then slot 1) |
| `$pred_taken` | Stays per slot; reads the shared counter via `\|fetch>>2$BranchState[1]` |
| `$pred_taken_branch` | Stays per slot; added to `$slot0_redir_e2` so slot 0 kills its slot-1 neighbour |
| `$pred_taken_branch_cond` | Split into `_0` / `_1`; slot 1 gains the `$slot0_redir_e2` qualifier |
| Good-path mask term | `4'b1110` for both slots (non-aborting redirect) |
| Next-PC arbiter | Two entries added below the replays, each with its own slot's `$branch_target` |
| `$branch_redir_pc` | Direction-aware; uses the per-slot `$pc_inc` |

No new intra-group hazard was introduced. The predictor is the one shared structure that both slots may update in the same cycle without contending — which is what distinguishes it from the memory and CSR ports.
...
Note that the branch predictor deliberately contributes **no fifth detector**: as [Section 6.11](#611-porting-the-two-bit-saturating-branch-predictor) explains, its counter accepts both slots' updates in the same cycle, so it is the one shared structure that never needs a structural hazard.
...
| **Branch predictor** | Two branches in one group | *None needed* | Both updates applied in series |
...
- **Slot 0 redirects.** Slot 1 of the same group lies on the wrong path. If the redirect is *aborting* (a trap), the mask clears the group's own bit and both slots die. If it is *non-aborting* (jump, predicted-taken branch, mispredicted branch, non-pipelined bubble), the group remains good so slot 0 commits, and `$killed_by_slot0` removes slot 1 alone.
...
> **A branch can now redirect twice.** With the two-bit predictor a single branch may steer the program counter at stage 2 (predicted taken) and again at stage 3 (mispredicted). The two never conflict: the stage-3 mispredict has strictly higher priority in the next-PC multiplexer and its mask term clears one bit more, so it cleanly overwrites the speculative fetch that its own earlier prediction started.

### 7.7 Predictor behaviour under dual issue

Two properties follow from sharing one counter between two slots:

- **Both slots of a group predict identically.** They read the same counter in the same cycle, so if a group holds two branches both receive the same prediction. Only slot 0's prediction can take effect, because a predicted-taken branch in slot 0 kills slot 1 outright.
- **Predictor state remains exact.** Because the two update steps are chained in program order and both are gated on `$commit`, the counter after any cycle equals the value a single-issue core would hold after retiring the same branches. Widening the machine changes prediction *timing*, never predictor *state*.
...
| **Predictor training** | `$BranchState` climbing toward `2'b11` as the loop branch is repeatedly taken, then stepping down on loop exit. |
| **Predicted-taken redirect** | `$pred_taken_branch_cond_0` firing at stage 2, with fetch following the branch target before the branch has resolved. |
